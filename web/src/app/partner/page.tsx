...
const RING =
  "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-timber";
...
export default function PartnerPage() {
  ...
  return (
    <main className="paper-noise flex min-h-dvh flex-col items-center bg-gradient-to-b from-timber via-ink to-ink-strong px-6 py-10 pb-[calc(2.5rem_+_env(safe-area-inset-bottom))] text-center text-paper">
      <button
        onClick={toggle}
        className={`self-end rounded-full border border-gold/40 px-3 py-1 text-xs font-semibold hover:bg-gold/10 ${RING}`}
      >
        {t("lang.switch")}
      </button>
      ...
      {phase === "booting" && (
        <p role="status" className="mt-6 animate-pulse text-paper/70">
          {t("common.loading")}
        </p>
      )}
      {phase === "need-token" && (
      ...
      {phase === "idle" && (
        <>
          <button
            ...
            className={`btn-primary mt-8 px-8 py-3.5 active:scale-[0.99] ${RING}`}
          >
          ...
          <p
            role="status"
            className="mt-4 max-w-xs text-sm leading-relaxed text-paper/70"
          >
      ...
      {phase === "scanning" && (
        <>
          ...
          <button
            ...
            className={`mt-4 rounded-full border border-paper/40 px-5 py-3 text-sm font-semibold text-paper/85 hover:bg-white/10 ${RING}`}
          >
            {t("partner.stop_scan")}
          </button>
        </>
      ...
      {phase === "found" && (
        <p role="status" className="font-display mt-4 text-lg font-bold text-gold">
          {t("partner.found")}
        </p>
      )}
      {phase === "claiming" && (
        <p role="status" className="mt-6 animate-pulse text-paper/85">
          {t("partner.claiming")}
        </p>
      )}
      {phase === "already" && (
      ...
      {apiError && !reveal && phase !== "claiming" && (
        <p role="alert" className="mt-4 text-sm text-wine-soft">
          {t("common.error")}
        </p>
      )}
      ...
      {showCollectionLink && (
        <Link
          href="/treasure"
          className={`mt-8 rounded-2xl border border-gold/50 px-6 py-3 font-semibold text-gold hover:bg-gold/10 ${RING}`}
        >
      ...
      {showManual && (
        <button
          onClick={manualClaim}
          className={`mt-6 rounded-full border border-paper/40 px-5 py-3 text-sm font-semibold text-paper/85 hover:bg-white/10 ${RING}`}
        >
